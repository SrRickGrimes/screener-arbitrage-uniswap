...
abstract contract UniswapFlashSwapper {
    ...
    function uniswapV2Call(
        ...
        (
            ...
            bytes memory _userData
                ...
                _data,
                (SwapType, address, uint, address, bool, bool, bytes, bytes)
            );
    ...
    function simpleFlashSwapExecute(
        ...
        address pairAddress = permissionedPairAddress; // gas efficiency
        uint pairBalanceTokenBorrow = IERC20(_tokenBorrow).balanceOf(
            pairAddress
        );
        uint pairBalanceTokenPay = IERC20(_tokenPay).balanceOf(pairAddress);
        uint amountToRepay = ((1000 * pairBalanceTokenPay * _amount) /
    ...
    function traingularFlashSwap(
        ...
        uint pairBalanceTokenBorrowBefore = IERC20(_tokenBorrow).balanceOf(
            borrowPairAddress
        );
        require(pairBalanceTokenBorrowBefore >= _amount, "_amount is too big");
        uint pairBalanceTokenBorrowAfter = pairBalanceTokenBorrowBefore -
            _amount;
        uint pairBalanceWeth = IERC20(WETH).balanceOf(borrowPairAddress);
        uint amountOfWeth = ((1000 * pairBalanceWeth * _amount) /
    ...
    function traingularFlashSwapExecute(
        ...
        (address _borrowPairAddress, uint _amountOfWeth) = abi.decode(
            _triangleData,
            (address, uint)
        );
        ...
        uint amountToRepay = ((1000 * pairBalanceTokenPay * _amountOfWeth) /
